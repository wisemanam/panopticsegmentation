...
class seg_decoder_bn(nn.Module):
    def __init__(self, num_classes):
        super(seg_decoder_bn, self).__init__()
    ...
    def forward(self, feature_map, skip_8, skip_4):
        ...
        skip_8 = F.relu(self.conv_1x1_2(skip_8)) # (shape: (batch_size, num_classes, h/8, w/8))
        ...
        # Should be skip connection:
        skip_4 = F.relu(self.conv_1x1_3(skip_4)) # (shape: (batch_size, num_classes, h/4, w/4))
        out = torch.cat((out, skip_4), 1)

        out = F.relu(self.bn_conv_5x5_2(self.conv_5x5_2(out))) # (shape: (batch_size, num_classes, h/4, w/4))

        # Prediction:
        out = self.conv_5x5_3(out) # (shape: (batch_size, num_classes, h/4, w/4))
        out = self.conv_1x1_4(out) # (shape: (batch_size, num_classes, h/4, w/4))
        return out

class inst_decoder_bn(nn.Module):
    def __init__(self, num_classes):
        super(inst_decoder_bn, self).__init__()
    ...
    def forward(self, feature_map, skip_8, skip_4):
        ...
        out = F.relu(self.conv_5x5_1(out)) # (shape: (batch_size, num_classes, h/8, w/8))
        out = F.upsample(out, size=(feature_map_h*4, feature_map_w*4), mode="bilinear") # (shape: (batch_size, num_classes, h/4, w/4))
        
        skip_4 = F.relu(self.conv_1x1_3(skip_4)) # (shape: (batch_size, num_classes, h/4, w/4))
        out = torch.cat((out, skip_4), 1)
        
        out = F.relu(self.conv_5x5_2(out)) # (shape: (batch_size, num_classes, h/4, w/4))
        
        # Prediction:
        out_center = self.conv_5x5_3(out)
        out_center = self.conv_1x1_4(out_center)

        out_regression = self.conv_5x5_3(out)
        out_regression = self.conv_1x1_5(out_regression)
...
class seg_decoder(nn.Module):
    ...
    def forward(self, feature_map, skip_8, skip_4):
        ...
        out = F.relu(self.conv_1x1_1(feature_map))  # (shape: (batch_size, num_classes, h/16, w/16))
        out = F.upsample(out, size=(feature_map_h*2, feature_map_w*2), mode="bilinear") # (shape: (batch_size, num_classes, h/8, w/8))
        # print('Shape after 1x1 conv and upsample: ', out.shape)        
        
        # Should be skip connection:
        skip_8 = F.relu(self.conv_1x1_2(skip_8)) # (shape: (batch_size, num_classes, h/8, w/8))
        out = torch.cat((out, skip_8), 1)
        # print('Shape after skip connection: ', out.shape)
        
        out = F.relu(self.conv_5x5_1(out)) # (shape: (batch_size, num_classes, h/8, w/8))
        out = F.upsample(out, size=(feature_map_h*4, feature_map_w*4), mode="bilinear") # (shape: (batch_size, num_classes, h/4, w/4))
        # print('Shape after 5x5 conv and upsample: ', out.shape)        
...
class inst_decoder(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv_1x1_2 = nn.Conv2d(512, 32, kernel_size=1) # skip_8

        self.conv_1x1_3 = nn.Conv2d(256, 16, kernel_size=1) # skip_4
        ...
        self.conv_5x5_4 = nn.Conv2d(128, 32, kernel_size=5)
    ...
    def forward(self, feature_map, skip_8, skip_4):
        ...
        # Center prediction:
        out_center = F.relu(self.conv_5x5_3(out))
        out_center = self.conv_1x1_4(out_center)

        # Regression prediction:
        out_regression = F.relu(self.conv_5x5_4(out))
        out_regression = self.conv_1x1_5(out_regression)
