...
class inst_decoder(nn.Module):
    ...
    def forward(self, feature_map, skip_8, skip_4):
        ...
        out = F.relu(self.conv_1x1_2(out)) # (shape: (batch_size, num_classes, h/8, w/8))
        out = F.upsample(out, size=(feature_map_h*4, feature_map_w*4), mode="bilinear") # (shape: (batch_size, num_classes, h/4, w/4))
        ...
        out_center = F.relu(self.conv_1x1_4(out))

        out_regression = F.relu(self.conv_5x5_3(out))
        out_regression = F.relu(self.conv_1x1_5(out))
