...
class CapsuleModel3(nn.Module):
    def __init__(self, model_id, project_dir):
        ...
        self.vote_dim = config.vote_dim
        self.vote_transform_seg = VotingModule(self.n_init_capsules, self.init_capsule_dim, self.vote_dim)
        self.vote_transform_class = VotingModule(self.n_init_capsules, self.init_capsule_dim, self.vote_dim)
        ...
        self.transformer_routing_seg = TransformerRouting(n_feats_in=self.vote_dim, n_caps_out=2, output_dim=16, use_vote_transform=False)
    ...
    def forward(self, x, point_lists=None, gt_seg=None, gt_reg=None):
        ...
        capsule_votes_seg_up = F.upsample(capsule_votes_seg, size=(h, w), mode="bilinear")  # can change this to h/N, w/N, N=2, 4, 8 if memory is an issue
        capsule_acts_up = F.upsample(capsule_acts, size=(h, w), mode="bilinear")

        b_size, _, h_new, w_new = capsule_votes_seg_up.shape
        capsule_votes_seg = capsule_votes_seg_up.permute(0, 2, 3, 1).view(b_size, h_new, w_new, self.n_init_capsules, self.vote_dim)
        capsule_acts_seg = capsule_acts_up.permute(0, 2, 3, 1)
...
if __name__ == '__main__':
    tester = CapsuleModel3('CapsuleModel3', 'SimpleSegmentation/').cuda()
